...
8. Tests S012 ajoutés/renforcés:
   ...
   - `app/tests/edge/phase-gate-governance-journal.edge.test.js`
   - `app/tests/e2e/phase-gate-governance-journal.spec.js`
9. Correctif reviewer AC-06 (fail-closed):
   - timestamps (`decidedAt`/`timestamp`) bornés à la plage JS Date (`±8_640_000_000_000_000` ms),
   - payload timestamp explicite hors plage -> `INVALID_GOVERNANCE_DECISION_INPUT` (sans exception non contrôlée),
   - entrées historiques hors plage ignorées proprement sans crash.
10. Durcissement délégation S011:
   - exceptions levées par `progressionAlertEvaluator` capturées,
   - conversion fail-closed en `INVALID_GOVERNANCE_DECISION_INPUT` avec raison explicite.
...
- Focus reviewer AC-06 (fail-closed):
  - `tests/edge/phase-gate-governance-journal.edge.test.js` couvre timestamps hors plage (`decidedAt`/`timestamp`) + exceptions `progressionAlertEvaluator`.
...
Revalidation checkpoint reviewer AC-06 (commande fast requise):
