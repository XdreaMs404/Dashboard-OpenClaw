# S009 - Override exceptionnel de transition avec justification et approbateur nominatif
...
## Contraintes
- Scope strict S009: pas de modification produit hors FR-009.
...
6. Export public S009 ajouté dans `app/src/index.js` (`evaluatePhaseTransitionOverride`).
7. Ajout/renforcement des tests S009:
   - `app/tests/unit/phase-transition-override.test.js`
   - `app/tests/edge/phase-transition-override.edge.test.js`
   - `app/tests/e2e/phase-transition-override.spec.js`
8. Démonstrateur e2e S009 couvrant explicitement `empty/loading/error/success`, rendu `reasonCode/reason/override.required/override.applied/requiredActions` + vérification responsive sans overflow.
...
- AC-09 (coverage >=95% lignes+branches): `npm run test:coverage` (S009 `phase-transition-override.js` = **99.24% lines**, **98.57% branches**)
