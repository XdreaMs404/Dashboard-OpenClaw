# S003 - Alerte de dépassement SLA de transition + plan d’action corrective
...
- [x] **AC-08 — Seuil qualité module**
      Couverture module S003 >= **95% lignes** et **95% branches**.
...
- [x] Aucune exécution shell dans S003.
...
2. Exporter S003 dans `app/src/index.js` (export S003 uniquement).
3. Ajouter/adapter les tests S003:
   - `app/tests/unit/phase-sla-alert.test.js`
   - `app/tests/edge/phase-sla-alert.edge.test.js`
   - `app/tests/e2e/phase-sla-alert.spec.js`
4. Mettre à jour la story S003 + handoffs DEV (`S003-dev-to-uxqa.md`, `S003-dev-to-tea.md`) avec preuves d’exécution.

### Interdit (hors-scope)
- Toute évolution hors FR-008 / S003.
- Toute modification métier S001..S006 non strictement nécessaire à l’intégration S003.
- Tout refactor transverse non requis par les AC S003.
- Toute exécution shell depuis le module S003.

## Contraintes
- Scope strict S003: pas de modification produit hors FR-008.
...
6. Export public S003 vérifié dans `app/src/index.js` (`evaluatePhaseSlaAlert`).
7. Renforcement/maintien des tests S003:
   - `app/tests/unit/phase-sla-alert.test.js`
   - `app/tests/edge/phase-sla-alert.edge.test.js`
   - `app/tests/e2e/phase-sla-alert.spec.js`
8. Démonstrateur e2e S003 validant explicitement les états UI `empty/loading/error/success`, reason codes, severity et correctiveActions + contrôle responsive sans overflow.
...
- AC-08 (coverage >=95% lignes+branches): `npm run test:coverage` (S003 `phase-sla-alert.js` = **100% lines**, **97.05% branches**)
