# S008 - Historique consultable des transitions de phase et verdicts associés
...
- FR-006 (réutilisation) : exploiter le verdict de l’orchestrateur guards sans ré-exécuter les scripts dans S008.
...
- [x] AC-09 (Seuil qualité) : couverture du module S008 >= 95% (lignes + branches).
...
- [x] Aucune exécution shell dans S008 (consommation read-only du verdict S005).
- [x] Messages de blocage explicites et actionnables pour UI/opérations.

## Contraintes
- Scope strict S008 : aucune dérive fonctionnelle hors FR-007.
- Réutiliser les reason codes S002/S004/S005 sans redéfinition contradictoire.
- S008 ne déclenche pas l’exécution des scripts guards; il consomme `guardResult` fourni.
...
9. Démonstrateur e2e S008 couvrant `empty/loading/error/success`, rendu explicite `reasonCode/reason/history (transition + verdict + timestamp)` + vérification responsive sans overflow.
...
- AC-09: `npm run test:coverage` (S008 `phase-transition-history.js` = **100% lines**, **98.52% branches**)
