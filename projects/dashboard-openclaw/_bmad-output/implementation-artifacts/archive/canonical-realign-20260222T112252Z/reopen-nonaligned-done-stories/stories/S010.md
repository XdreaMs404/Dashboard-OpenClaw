# S010 - Matrice des dépendances bloquantes inter-phases en temps réel
...
## Contraintes
- Scope strict S010: pas de modification produit hors FR-010.
...
6. Contrat de sortie stable garanti:
   `{ allowed, reasonCode, reason, diagnostics, dependencies, blockingDependencies, correctiveActions }`.
7. Export public S010 ajouté dans `app/src/index.js` (`buildPhaseDependencyMatrix`).
...
- AC-10 (coverage >=95% lignes+branches): `npm run test:coverage` (S010 `phase-dependency-matrix.js` = **99.63% lines**, **99.23% branches**)
