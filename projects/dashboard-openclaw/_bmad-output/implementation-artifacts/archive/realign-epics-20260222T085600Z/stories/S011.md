# S011 - Pipeline d’ingestion d’artefacts markdown/yaml sous allowlist (FR-021/FR-022)
...
### Interdit (hors-scope)
- Toute évolution hors FR-021/FR-022 / S011.
- Toute modification métier S001..S010 non strictement nécessaire à l’intégration S011.
...
7. Implémentation du contrat de sortie stable:
   `{ allowed, reasonCode, reason, diagnostics, ingestedArtifacts, rejectedArtifacts, correctiveActions }`.
8. Export S011 ajouté dans `app/src/index.js` (`ingestBmadArtifacts`) sans casser les exports existants.
9. Ajout/renforcement des tests S011:
   - `app/tests/unit/artifact-ingestion-pipeline.test.js`
   - `app/tests/edge/artifact-ingestion-pipeline.edge.test.js`
   - `app/tests/e2e/artifact-ingestion-pipeline.spec.js`
...
- AC-10 (coverage >=95% lignes+branches): `npm run test:coverage` (S011 `artifact-ingestion-pipeline.js` = **100% lines**, **100% branches**)
