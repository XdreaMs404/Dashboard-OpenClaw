...
- Implémentation limitée au scope PM S008 (FR-007) : historisation consultable des transitions + filtres + rétention.
- Aucune exécution shell dans S008 ; consommation read-only de `guardResult` (sortie S005).
- Contrat stable S008 livré :
...
5. `npm run test:coverage` ✅
   ...
   - module S008 `app/src/phase-transition-history.js` : **100% lines / 98.52% branches**
6. `npm run build` ✅
...
- AC-09 (coverage >=95% lignes+branches): `test:coverage` PASS (**100% lines / 98.52% branches** sur S008)
