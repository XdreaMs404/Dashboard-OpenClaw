# H13 — PM → DEV — S016 (scope strict)

## Contexte
- **SID**: S016
- **Epic**: E02
- **Projet**: `/root/.openclaw/workspace/projects/dashboard-openclaw`
- **Story source (SoT)**: `_bmad-output/implementation-artifacts/stories/S016.md`
- **Références planning**:
...
1. Story S016 cadrée, testable et traçable (FR/NFR/risques/AC/cas de test/contraintes).
...
4. Périmètre DEV verrouillé en **scope strict S016 uniquement**.

## Décision PM
**GO_DEV explicite — S016 uniquement.**

## Scope DEV autorisé (strict S016)
...
4. Exposer la préparation S015 via `searchIndexEligible=true`, sans implémenter la recherche full-text dans S016.
...
6. Ajouter/maintenir les tests S016 unit + edge + e2e, avec couverture >=95% lignes/branches et benchmark 500 docs conforme.
7. Exporter S016 dans `app/src/index.js` (export S016 uniquement).
...
## Fichiers cibles autorisés (strict S016)
- `app/src/artifact-table-indexer.js`
- `app/src/index.js` (export S016 uniquement)
...
- Toute modification fonctionnelle S001..S013 non strictement nécessaire à l’intégration S016.
...
3. Couverture S016 (`artifact-table-indexer.js`) >=95% lignes + branches.
4. Preuve performance 500 docs (`p95IndexMs`, durée totale).
5. Preuve de non-régression S001..S013.
6. Liste des fichiers modifiés strictement limitée au scope autorisé S016.
