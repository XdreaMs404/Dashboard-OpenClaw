...
const roles = {
  'bmad-brainstorm': {
    ...
    handoff: [
      'Vers Analyst: inputs de brainstorming consolidés (H03)',
      'Vers PM: options décisionnelles + critères de succès (H03)',
      'Vers Jarvis: statut complétude analysis côté H01'
    ],
    forbidden: [
  ...
  'bmad-analyst': {
    ...
    outputs: [
      ...
      'Minimum 3 axes de recherche approfondis (implémentation, concurrence/benchmark, risques/contraintes)'
    ],
    qualityGates: [
      ...
      'Au moins 3 recherches utiles et exploitables sont produites et sourcées'
    ],
    handoff: [
      'Vers PM: product-brief consolidé + priorisation (H03)',
      'Vers Jarvis: synthèse des 3 recherches utiles pour Gate G1',
      'Vers Brainstorming/PM: clarification des inconnues critiques'
    ],
    forbidden: [
  ...
  'bmad-pm': {
    ...
    principles: [
      ...
      'Interdit de reporter des exigences critiques à une version future: viser la meilleure version livrable dès le départ.',
    ...
    outputs: [
      'PRD structuré et actionnable',
      'AC précis + testabilité explicite',
      'Préparation solutioning PM+UX pour Architect',
      'Handoffs H05/H07/H08/H10 tracés'
    ],
    qualityGates: [
    ...
    handoff: [
      'Vers UX Designer: PRD + attentes UX explicites (H05)',
      'Vers Architect: inputs PM+UX consolidés (H08/H10)',
      'Vers Jarvis: validation planning (H07) et suivi adaptations post-rétro (H22)'
    ],
    forbidden: [
  ...
  'bmad-ux-designer': {
    ...
    handoff: [
      'Vers PM/Architect: contraintes UX exploitables + standards UI obligatoires (H06)',
      'Vers Jarvis (avec PM): validation de complétude planning (H07)',
      'Vers Architect (avec PM): entrée solutioning (H08)'
    ],
    forbidden: [
  ...
  'bmad-architect': {
    ...
    handoff: [
      'Vers PM: architecture détaillée + impacts + dépendances (H09)',
      'Vers Architect (readiness): verdict PASS/CONCERNS/FAIL (H10)',
      'Vers Jarvis (indirect via PM): état de préparation implémentation'
    ],
    forbidden: [
      'Over-engineering',
  ...
  'bmad-sm': {
    ...
    handoff: [
      'Vers DEV: story pack prêt + AC + contraintes (H13)',
      'Vers Jarvis: update status + next story (H19)',
      'Vers Jarvis: epic candidate complete (H20)'
    ],
    forbidden: [
...
const MATRIX_SOURCE = '/root/.openclaw/workspace/docs/BMAD-HYPER-ORCHESTRATION-THEORY.md';

const ROLE_ROUTE = {
  'bmad-brainstorm': {
    phase: 'H01',
    handoffIn: ['Jarvis -> Brainstorming Coach (kickoff analysis)'],
    handoffOut: ['H03 Analyst + Brainstorming -> PM (inputs brainstorming consolidés)']
  },
  'bmad-analyst': {
    phase: 'H02-H03',
    handoffIn: ['H02 Jarvis -> Analyst (market + tech + risk research)'],
    handoffOut: ['H03 Analyst + Brainstorming -> PM (product-brief consolidé)']
  },
  'bmad-pm': {
    phase: 'H03-H04-H05-H07-H08-H09-H10-H22',
    handoffIn: [
      'H03 Analyst + Brainstorming -> PM',
      'H06 UX Designer -> PM/Architect',
      'H09 Architect -> PM',
      'H22 Retro -> PM/Architect/UX'
    ],
    handoffOut: [
  ...
  'bmad-ux-designer': {
    phase: 'H05-H06-H07-H08-H22',
    handoffIn: ['H05 PM -> UX Designer', 'H22 Retro -> PM/Architect/UX'],
    handoffOut: [
      'H06 UX Designer -> PM/Architect (contraintes UX exploitables + standards UI obligatoires)',
      'H07 PM + UX -> Jarvis (validation de complétude)',
      'H08 PM + UX -> Architect'
    ]
  },
  'bmad-architect': {
    phase: 'H08-H09-H10-H22',
    handoffIn: ['H08 PM + UX -> Architect', 'H22 Retro -> PM/Architect/UX'],
    handoffOut: [
      'H09 Architect -> PM (epics/*.md, epics.md, epics-index.md)',
      'H10 PM + Architect -> Architect (PASS/CONCERNS/FAIL)'
    ]
  },
  'bmad-sm': {
    phase: 'H11-H12-H13-H19-H20-H21',
    handoffIn: [
  ...
  'bmad-dev': {
    phase: 'H13-H14-H16',
    handoffIn: ['H13 SM -> DEV (story pack prêt)', 'H15 UX QA Auditor -> DEV/TEA (corrections)'],
    handoffOut: [
  ...
  'bmad-tea': {
    phase: 'H15-H16-H17-H21',
    handoffIn: [
      'H15 UX QA Auditor -> DEV/TEA (verdict UX + corrections)',
      'H16 DEV -> TEA (evidence test)',
      'H21 Jarvis -> SM/TEA/UX QA (retro élargie)'
    ],
    handoffOut: ['H17 TEA -> Reviewer (statut qualité + gaps)']
  },
  'bmad-reviewer': {
    phase: 'H17-H18',
    handoffIn: ['H17 TEA -> Reviewer (statut qualité + gaps)'],
    handoffOut: ['H18 Reviewer -> Jarvis (approve / changes / blocked)']
  },
  'bmad-ux-qa': {
    phase: 'H14-H15-H21',
    handoffIn: ['H14 DEV -> UX QA Auditor (evidence UX/UI + captures + states)', 'H21 Jarvis -> SM/TEA/UX QA (retro élargie)'],
    handoffOut: ['H15 UX QA Auditor -> DEV/TEA (verdict UX + corrections)']
  },
  'bmad-tech-writer': {
    phase: 'Support transverse post-H18',
    handoffIn: ['Après H18, une fois verdict reviewer consolidé'],
    handoffOut: ['Résumé final vers Alex + archivage preuves']
  }
};

function roleRoute(id) {
  return ROLE_ROUTE[id] || {
    phase: 'N/A',
    handoffIn: ['Non défini'],
    handoffOut: ['Non défini']
  };
}
...
function renderIdentity(id, r) {
  const route = roleRoute(id);
  return `# IDENTITY.md
- Name: ${r.name}
- Emoji: ${r.emoji}
- Role: ${r.role}
- Mission: ${r.mission}
- Vibe: ${r.vibe}
- Phase canonique: ${route.phase}
- Handoff IN: ${route.handoffIn.join(' | ')}
- Handoff OUT: ${route.handoffOut.join(' | ')}
- Source BMAD: ${r.sourceRefs[0] || 'N/A'}
- Matrice de référence: ${MATRIX_SOURCE}
`;
}
...
- Politique produit: viser la meilleure version livrable directement, sans report en version future.
- Protocole global: /new /start /pause /continue /recap (orchestration pilotée par Jarvis).
`;
}

function renderSoul(id, r) {
  const route = roleRoute(id);
  ...
  const handoffIn = route.handoffIn.map(x => `- ${x}`).join('\n');
  const handoffOut = route.handoffOut.map(x => `- ${x}`).join('\n');
...
## Position dans l’orchestration canonique
- Phase/Handoff: ${route.phase}
- Source de vérité: ${MATRIX_SOURCE} (matrice H01 -> H23 + gates G1 -> G5)
- Règle absolue: ne pas reporter d’exigence critique à une version future; viser la meilleure version livrable dès le premier cycle.

## Style
${r.communicationStyle}

## Protocole commandes (pilotage Jarvis)
- /new : cadrage uniquement
- /start : exécution phases 1→3
- fin phase 3 : attente validation explicite
- /continue : autorise reprise phase 4+
- /pause : arrêt immédiat
- /recap : état synthétique
...
## Contrat de handoff (entrée)
${handoffIn}

## Contrat de handoff (sortie)
${handoffOut}
...
${startup ? '\n' : ''}${startup.split('\n').length + 2}. Vérifier que la mission respecte strictement l’ordre H01 -> H23 et qu’aucune exigence critique n’est reportée à plus tard.
...
## 2) Position canonique dans la matrice
- Segment: ${route.phase}
- Source de vérité unique: ${MATRIX_SOURCE}
- Ordre obligatoire global: H01 -> H23, sans permutation.

## 3) Entrées attendues
${inputs}

## 4) Sorties obligatoires
${outputs}

## 5) Gates qualité (avant handoff)
${gates}

## 6) Contrat de handoff (spécifique rôle)
### Handoff entrant
${handoffIn}
...
### Compléments de transmission
${handoff}

## 7) Interdits absolus
${forbidden}
- Interdit de réduire le scope en reportant des exigences critiques à une version ultérieure.

## 8) Routines d’exécution BMAD
...
## 9) Protocole global commandes (piloté par Jarvis)
- /new : cadrage exhaustif uniquement (pas d’exécution autonome).
- /start : exécution autonome phases 1→3 (H01→H10).
- Fin phase 3 : retour idle + attente validation explicite.
- /continue : autorise phase 4+ après validation utilisateur.
- /pause : arrêt immédiat.
- /recap : état réel synthétique.

## 10) Alignement projet local
...
for (const [id, role] of Object.entries(roles)) {
  ...
  writeFile(path.join(dir, 'IDENTITY.md'), renderIdentity(id, role));
  writeFile(path.join(dir, 'USER.md'), renderUser(role));
  writeFile(path.join(dir, 'BMAD_SOURCE_PROMPT.md'), renderSourcePromptPack(role));
  writeFile(path.join(dir, 'SOUL.md'), renderSoul(id, role));
  writeFile(path.join(dir, 'AGENTS.md'), renderAgents(id, role));
