...
for eid in sorted(epics.keys()):
    epic_num=int(eid[1:]) if eid and eid[1:].isdigit() else 0
    out.append(f'  epic-{epic_num}: backlog')
    for sid,title in epics[eid]:
      sid_num = int(sid[1:]) if sid and sid[1:].isdigit() else 0
      skey=f"{epic_num}-{sid_num:02d}-{title.lower().replace(' ','-')}"
...
if [[ -x "$ROOT/scripts/enforce-story-template.sh" ]]; then
  bash "$ROOT/scripts/enforce-story-template.sh"
elif [[ -x "$CONTROL_ROOT/scripts/enforce-story-template.sh" ]]; then
  BMAD_PROJECT_ROOT="$ROOT" bash "$CONTROL_ROOT/scripts/enforce-story-template.sh"
fi

bash "$CONTROL_ROOT/scripts/set-mode.sh" idle
bash "$CONTROL_ROOT/scripts/set-lifecycle.sh" ideation false "/new" "Reset projet"

echo "Project reset complete for: $ROOT"
