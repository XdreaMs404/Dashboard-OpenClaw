...
REGISTRY_FILE="$PROJECT_ROOT/runtime/agent-registry.json"
...
[[ -d "$PROJECT_ROOT" ]] || fail "Active project root not found: $PROJECT_ROOT"
...
for script in \
  project-env.sh project-create.sh project-list.sh project-switch.sh \
  set-mode.sh set-lifecycle.sh control-command.sh phase-complete.sh progress.sh next-story.sh runtime-healthcheck.sh \
...
PROJECT_REL="${PROJECT_ROOT#/root/.openclaw/workspace/}"
...
if command -v git >/dev/null 2>&1; then
  ...
  dirty_count="$(git -C /root/.openclaw/workspace status --porcelain -- "$PROJECT_REL" | wc -l | tr -d ' ')"
  if [[ "$dirty_count" -gt 0 ]]; then
    warn "git pending changes under $PROJECT_REL: $dirty_count (next story auto-commit will include them)"
  else
    ok "git working tree clean for $PROJECT_REL"
  fi

  if [[ -d "$PROJECT_ROOT/runtime/git-sync" ]]; then
    pending_count="$(find "$PROJECT_ROOT/runtime/git-sync" -type f -name '*push-failed*' | wc -l | tr -d ' ')"
...
if [[ -f "$STATUS_FILE" ]]; then
  ...
  ACTIVE_FIELD="$(awk -F': ' '/^active_project_root:/ {print $2}' "$STATUS_FILE" | tr -d '\r' || true)"
  if [[ "$MODE" == "active" || "$MODE" == "idle" ]]; then
    ok "PROJECT_STATUS mode=$MODE"
  else
    fail "Invalid PROJECT_STATUS mode: '${MODE:-<empty>}'"
  fi
  if [[ -n "$ACTIVE_FIELD" ]]; then
    if [[ "$ACTIVE_FIELD" == "$PROJECT_ROOT" ]]; then
      ok "PROJECT_STATUS active_project_root matches runtime"
    else
      warn "PROJECT_STATUS active_project_root differs: $ACTIVE_FIELD"
    fi
  else
    warn "PROJECT_STATUS active_project_root missing"
  fi
fi
...
print('has_lifecycle_gate=' + ('true' if 'lifecycle_state is not `phase4_implementation`' in payload else 'false'))
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    has_lifecycle_gate=true) ok "Cron payload lifecycle gate present (phase4 only)" ;;
    has_lifecycle_gate=false) warn "Cron payload lifecycle gate missing" ;;
