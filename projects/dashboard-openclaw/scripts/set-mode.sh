...
for line in text.splitlines():
    ...
    elif line.startswith('active_project_root:'):
        lines.append('active_project_root: $ACTIVE_PROJECT_ROOT'); seen.add('active_project_root')
...
if 'active_project_root' not in seen: lines.append('active_project_root: $ACTIVE_PROJECT_ROOT')
if 'last_update' not in seen: lines.append('last_update: '+datetime.now(timezone.utc).isoformat())

p.write_text('\n'.join(lines)+'\n', encoding='utf-8')
PY

echo "PROJECT_STATUS updated: mode=$MODE active_project_root=$ACTIVE_PROJECT_ROOT"
