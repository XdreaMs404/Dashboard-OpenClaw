...
if (!/^S\d{3}$/.test(sid)) {
  console.error('Usage: node scripts/capture-aqcd-snapshot-ux-evidence.mjs S050');
  process.exit(1);
}
...
function buildReasonCopyChecks() {
  ...
  const success = buildAqcdSnapshotComparisons(buildPayload(), {
    nowMs: Date.parse('2026-03-03T01:00:00.000Z'),
    cadenceHours: 24
  });
...
for (const viewport of viewports) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    const overflow = {
      ...
      comparisons: getOverflow('#comparison-value'),
