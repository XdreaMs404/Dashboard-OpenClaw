...
<html lang="fr">
  <head>
    <meta charset="UTF-8" />
    <title>Ingestion artefacts BMAD</title>
    <style>
      :root {
        color-scheme: light;
  ...
  <body>
    <main>
      <h1>Pipeline ingestion d’artefacts</h1>
      ...
      <button id="run-action" type="button">Lancer ingestion</button>

      <p id="state-indicator" role="status" aria-label="État ingestion artefacts" aria-live="polite" data-state="empty">
        État: empty
      </p>
    ...
    <script>
      ...
      const renderResult = (result) => {
        ...
        countsValue.textContent =
          ...
          ' | ingested=' + String(diagnostics.ingestedCount ?? '—') +
          ' | rejected=' + String(diagnostics.rejectedCount ?? '—');
      ...
      action.addEventListener('click', async () => {
        ...
        try {
          ...
          const result = await window.runArtifactIngestionScenarioRuntime(scenarioInput.value);
          renderResult(result);
  ...
  if (scenario === 'success') {
    return ingestBmadArtifacts({
      ...
      artifactDocuments: [
        ...
        {
          path: `${ALLOWLIST_ROOT}/evidence.yaml`,
          content: 'owner: pm\nnotes:\n  - stable'
        }
      ]
    });
...
async function bootstrapDemoPage(page) {
  await page.exposeFunction('runArtifactIngestionScenarioRuntime', runScenario);
  await page.goto(`data:text/html,${encodeURIComponent(demoPageHtml)}`);
}

test('artifact ingestion demo covers empty/loading/error/success with explicit reason, counters and correctiveActions', async ({
  page
}) => {
  await bootstrapDemoPage(page);

  const scenario = page.getByLabel('Scénario');
  const action = page.getByRole('button', { name: 'Lancer ingestion' });
  const stateIndicator = page.getByRole('status', { name: 'État ingestion artefacts' });
  ...
  await expect(reasonCodeValue).toHaveText('INVALID_ARTIFACT_INGESTION_INPUT');
  await expect(reasonValue).toContainText('artifactDocuments ou artifactPaths est requis');
  await expect(countsValue).toContainText('requested=0 | ingested=0 | rejected=0');
  ...
  await expect(countsValue).toContainText('requested=1 | ingested=0 | rejected=1');
  ...
  await expect(reasonValue).toContainText('Ingestion réussie');
  await expect(countsValue).toContainText('requested=2 | ingested=2 | rejected=0');
  await expect(actionsValue).toContainText('—');
...
test('artifact ingestion demo remains readable without horizontal overflow on mobile/tablet/desktop', async ({
  browser
}) => {
  for (const viewport of responsiveViewports) {
    await test.step(`${viewport.name} viewport`, async () => {
      ...
      try {
        ...
        await page.getByRole('button', { name: 'Lancer ingestion' }).click();

        await expect(page.getByRole('status', { name: 'État ingestion artefacts' })).toHaveAttribute(
          'data-state',
          'error'
        );
