...
<html lang="fr">
  <head>
    <meta charset="UTF-8" />
    <title>Lint design tokens anti-style rogue</title>
    <style>
      :root {
        color-scheme: light;
  ...
  <body>
    <main>
      <h1>Lint design tokens anti-style rogue</h1>

      <label for="scenario">Scénario</label>
      <select id="scenario" name="scenario">
        <option value="invalid-input">Entrée invalide</option>
        <option value="missing-state">État critique manquant</option>
        <option value="rogue-style">Style rogue détecté</option>
        <option value="success">Nominal</option>
      </select>

      <button id="run-contract" type="button">Évaluer lint design tokens</button>

      <p id="state-indicator" role="status" aria-label="État lint design tokens" aria-live="polite" data-state="empty">
        État: empty
      </p>
      ...
      <section aria-label="Zone focus démonstration">
        ...
        <div class="focus-zone">
          <button id="focus-0" class="focus-target" type="button">Filtrer vue</button>
          <button id="focus-1" class="focus-target" type="button">Lancer lint</button>
          <button id="focus-2" class="focus-target" type="button">Exporter rapport</button>
        </div>
      </section>
      ...
      <section>
        ...
        <dl>
          ...
          <div><dt>surfaces</dt><dd id="surface-value">—</dd></div>
        </dl>
      </section>
    ...
    <script>
      ...
      const renderResult = (result) => {
        ...
        diagValue.textContent =
          'model=' + String(diagnostics.designTokenModelVersion ?? '—') +
          ' | views=' + String(diagnostics.criticalViewCount ?? '—') +
          ' | rogue=' + String(diagnostics.rogueSurfaceCount ?? '—') +
          ' | budget=' + String(diagnostics.decisionTimeBudgetSec ?? '—');

        const summary = result.designTokenRogueLint?.summary ?? {};
        summaryValue.textContent =
          'four-states=' + String(summary.fourStateCoveragePct ?? '—') +
          '% | responsive=' + String(summary.responsiveCoveragePct ?? '—') +
          '% | budget=' + String(summary.decisionTimeBudgetSec ?? '—') +
          's | worst=' + String(summary.worstDecisionTimeSec ?? '—') + 's';

        const surfaces = Array.isArray(result.designTokenRogueLint?.criticalViews)
          ? result.designTokenRogueLint.criticalViews
          : [];

        surfaceValue.textContent = surfaces
          .map((entry) => String(entry.id) + ':compliant=' + String(entry.compliant))
          .join(' | ');
      };
      ...
      action.addEventListener('click', async () => {
        ...
        try {
          ...
          const result = await window.runDesignTokenRogueLintScenarioRuntime(scenarioInput.value);
          renderResult(result);
...
function runScenario(scenario) {
  if (scenario === 'success') {
    return buildUxDesignTokenRogueLint(buildPayload(), {
  ...
  if (scenario === 'missing-state') {
    ...
    return buildUxDesignTokenRogueLint(payload, {
      nowMs: Date.parse('2026-03-20T09:00:00.000Z')
    });
  }

  if (scenario === 'rogue-style') {
    const payload = buildPayload();
  ...
  return buildUxDesignTokenRogueLint('bad-input');
}

test('design token rogue lint demo covers empty/loading/error/success with strict anti-rogue checks', async ({
  page
}) => {
  await page.exposeFunction('runDesignTokenRogueLintScenarioRuntime', runScenario);

  await page.goto(`data:text/html,${encodeURIComponent(demoPageHtml)}`);

  const scenario = page.getByLabel('Scénario');
  const action = page.getByRole('button', { name: 'Évaluer lint design tokens' });
  const stateIndicator = page.getByRole('status', { name: 'État lint design tokens' });

  const reasonCodeValue = page.locator('#reason-code-value');
  const summaryValue = page.locator('#summary-value');
  const surfaceValue = page.locator('#surface-value');
  ...
  await expect(reasonCodeValue).toHaveText('INVALID_UX_DESIGN_TOKEN_INPUT');

  await scenario.selectOption('missing-state');
  await action.click();
  await expect(stateIndicator).toHaveAttribute('data-state', 'error');
  await expect(reasonCodeValue).toHaveText('UX_CRITICAL_VIEW_FOUR_STATES_REQUIRED');

  await scenario.selectOption('rogue-style');
  await action.click();
  await expect(stateIndicator).toHaveAttribute('data-state', 'error');
  await expect(reasonCodeValue).toHaveText('UX_DESIGN_TOKEN_ROGUE_STYLE_DETECTED');

  await scenario.selectOption('success');
  await action.click();
  await expect(stateIndicator).toHaveAttribute('data-state', 'success');
  await expect(reasonCodeValue).toHaveText('OK');
  await expect(summaryValue).toContainText('four-states=100%');
  await expect(summaryValue).toContainText('responsive=100%');
  await expect(summaryValue).toContainText('budget=90s');
  await expect(surfaceValue).toContainText('gate-overview:compliant=true');
...
test('design token rogue lint demo remains readable without horizontal overflow on mobile/tablet/desktop', async ({
  page
}) => {
  await page.exposeFunction('runDesignTokenRogueLintScenarioRuntime', runScenario);
  ...
  await page.getByRole('button', { name: 'Évaluer lint design tokens' }).click();
  await expect(page.getByRole('status', { name: 'État lint design tokens' })).toHaveAttribute(
    'data-state',
    'success'
  );
