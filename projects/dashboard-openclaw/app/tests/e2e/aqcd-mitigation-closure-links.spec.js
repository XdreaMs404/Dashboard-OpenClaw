...
test('aqcd mitigation closure links demo covers empty/loading/error/success with linkage + heatmap details', async ({
  page
}) => {
  ...
  await expect(reasonCodeValue).toHaveText('INVALID_AQCD_MITIGATION_LINK_INPUT');

  await scenario.selectOption('closure-proof-missing');
  await runButton.click();
  await expect(stateIndicator).toHaveAttribute('data-state', 'error');
  await expect(reasonCodeValue).toHaveText('AQCD_MITIGATION_CLOSURE_PROOF_REQUIRED');
  await expect(errorMessage).toContainText('AQCD_MITIGATION_CLOSURE_PROOF_REQUIRED');

  await scenario.selectOption('missing-heatmap');
  await runButton.click();
  await expect(stateIndicator).toHaveAttribute('data-state', 'error');
  await expect(reasonCodeValue).toHaveText('AQCD_HEATMAP_EVOLUTION_REQUIRED');

  await scenario.selectOption('success');
  await runButton.click();
  await expect(stateIndicator).toHaveAttribute('data-state', 'success');
  await expect(reasonCodeValue).toHaveText('OK');
  await expect(linksValue).toContainText('total=2');
  await expect(linksValue).toContainText('closedWithProof=1');
  await expect(heatmapValue).toContainText('continuous=true');
  await expect(diagValue).toContainText('metricsContinuous=true');
  await expect(successJson).toContainText('mitigationClosureLinks');
});

test('aqcd mitigation closure links demo remains readable without horizontal overflow on mobile/tablet/desktop', async ({
  page
}) => {
  await page.exposeFunction('runMitigationClosureScenarioRuntime', runScenario);
