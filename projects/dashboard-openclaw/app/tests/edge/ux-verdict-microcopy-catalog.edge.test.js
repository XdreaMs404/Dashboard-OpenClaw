...
describe('ux-verdict-microcopy-catalog edge', () => {
  it('rejects malformed payload', () => {
    const result = buildUxVerdictMicrocopyCatalog('bad-input');

    expect(result.allowed).toBe(false);
    expect(result.reasonCode).toBe('INVALID_UX_VERDICT_MICROCOPY_INPUT');
  });

  it('propagates base failure when contextual glossary mapping is broken', () => {
  ...
  it('fails when keyboard flow is incomplete on one verdict', () => {
    const payload = buildPayload();
    ...
    expect(result.allowed).toBe(false);
    expect(result.reasonCode).toBe('UX_MICROCOPY_KEYBOARD_FLOW_REQUIRED');
  });

  it('fails when decision budget is exceeded', () => {
    const payload = buildPayload();
    payload.microcopyCatalog[1].decisionTimeSec = 121;
    ...
    expect(result.allowed).toBe(false);
    expect(result.reasonCode).toBe('UX_MICROCOPY_DECISION_BUDGET_EXCEEDED');
  });

  it('fails when TTFV exceeds threshold', () => {
    const payload = buildPayload();
    payload.microcopyCatalog[0].ttfvDays = 18;
    ...
    expect(result.allowed).toBe(false);
    expect(result.reasonCode).toBe('UX_MICROCOPY_TTFV_EXCEEDED');
  });

  it('fails when catalog is missing', () => {
    const payload = buildPayload();
    delete payload.microcopyCatalog;
    ...
    expect(result.allowed).toBe(false);
    expect(result.reasonCode).toBe('UX_MICROCOPY_CATALOG_REQUIRED');
  });

  it('accepts alias fields with runtime override and keyboard requirement disabled', () => {
    const payload = buildPayload();

    payload.copyCatalog = payload.microcopyCatalog.map((entry) => ({
      status: entry.verdict,
      label: entry.title,
      description: entry.helperText,
      cta: entry.actionLabel,
      states: {
        loading: { copy: entry.states.loading },
        empty: { copy: entry.states.empty },
        error: { copy: entry.states.error },
        success: { copy: entry.states.success }
      },
      keyboard: {
      ...
      timeToFirstValueDays: entry.ttfvDays
    }));
    delete payload.microcopyCatalog;
    ...
    const result = buildUxVerdictMicrocopyCatalog(payload, {
      ...
      microcopyCatalogRules: {
        modelVersion: 'S069-v1-runtime',
        requiredVerdicts: ['PASS', 'CONCERNS', 'FAIL', 'PASS'],
        requiredStates: ['loading', 'empty', 'error', 'success', 'loading'],
        requireKeyboardFlow: false,
        decisionTimeBudgetSec: 90,
        ttfvMaxDays: 14
      }
    });
    ...
    expect(result.verdictMicrocopyCatalog.modelVersion).toBe('S069-v1-runtime');
  });
});
