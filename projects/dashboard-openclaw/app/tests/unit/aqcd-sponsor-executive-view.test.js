...
import { buildAqcdSponsorExecutiveView as buildFromIndex } from '../../src/index.js';
...
function buildPayload() {
  return {
    ...
    metricSources: {
      autonomy: { A1: 'telemetry://autonomy/A1', A2: 'telemetry://autonomy/A2', A3: 'telemetry://autonomy/A3', A4: 'telemetry://autonomy/A4' },
      qualityTech: { Q1: 'telemetry://quality/Q1', Q2: 'telemetry://quality/Q2', Q3: 'telemetry://quality/Q3', Q4: 'telemetry://quality/Q4', Q5: 'telemetry://quality/Q5' },
      costEfficiency: { C1: 'telemetry://cost/C1', C2: 'telemetry://cost/C2', C3: 'telemetry://cost/C3', C4: 'telemetry://cost/C4' },
      design: { D1: 'telemetry://design/D1', D2: 'telemetry://design/D2', D3: 'telemetry://design/D3', D4: 'telemetry://design/D4', D5: 'telemetry://design/D5', D6: 'telemetry://design/D6' }
    },
    snapshots: [
      {
        id: 'AQCD-S057-W1',
...
describe('aqcd-sponsor-executive-view unit', () => {
  it('builds a simplified sponsor view with AQCD scores, formula sources and trend snapshots (S059/FR-045/FR-046)', () => {
    const result = buildAqcdSponsorExecutiveView(buildPayload(), {
      nowMs: Date.parse('2026-03-12T12:00:00.000Z')
    });

    expect(result.allowed).toBe(true);
    expect(result.reasonCode).toBe('OK');
    expect(result.sponsorExecutiveView).toMatchObject({
    ...
    expect(result.sponsorExecutiveView.scoreCards).toHaveLength(5);
    expect(result.sponsorExecutiveView.trend.snapshotCount).toBe(3);
    expect(result.sponsorExecutiveView.governance.metricsContinuous).toBe(true);
    expect(result.formula?.dimensions?.autonomy?.terms?.[0]?.source).toContain('telemetry://autonomy/');
  });

  it('fails when sponsor history has fewer snapshots than required', () => {
    ...
    expect(result.correctiveActions).toContain('ADD_SPONSOR_PERIODIC_SNAPSHOTS');
  });

  it('fails on snapshot continuity gap when sponsor continuity is enforced', () => {
    const payload = buildPayload();
