import { describe, expect, it } from 'vitest';
import { buildAqcdReadinessRules } from '../../src/aqcd-readiness-rules.js';
import { buildAqcdReadinessRules as buildAqcdReadinessRulesFromIndex } from '../../src/index.js';

function buildPayload() {
  return {
    window: 'story',
    windowRef: 'S051',
    metrics: {
      autonomy: { A1: 90, A2: 88, A3: 85, A4: 89 },
      qualityTech: { Q1: 86, Q2: 84, Q3: 82, Q4: 80, Q5: 88 },
      costEfficiency: { C1: 74, C2: 77, C3: 79, C4: 73 },
      design: { D1: 90, D2: 89, D3: 87, D4: 86, D5: 88, D6: 85 }
    },
    metricSources: {
      autonomy: {
        A1: 'telemetry://autonomy/A1',
        A2: 'telemetry://autonomy/A2',
        A3: 'telemetry://autonomy/A3',
        A4: 'telemetry://autonomy/A4'
      },
      qualityTech: {
        Q1: 'telemetry://quality/Q1',
        Q2: 'telemetry://quality/Q2',
        Q3: 'telemetry://quality/Q3',
        Q4: 'telemetry://quality/Q4',
        Q5: 'telemetry://quality/Q5'
      },
      costEfficiency: {
        C1: 'telemetry://cost/C1',
        C2: 'telemetry://cost/C2',
        C3: 'telemetry://cost/C3',
        C4: 'telemetry://cost/C4'
      },
      design: {
        D1: 'telemetry://design/D1',
        D2: 'telemetry://design/D2',
        D3: 'telemetry://design/D3',
        D4: 'telemetry://design/D4',
        D5: 'telemetry://design/D5',
        D6: 'telemetry://design/D6'
      }
    },
    snapshots: [
      {
        id: 'AQCD-S049-W1',
        windowRef: 'S049-W1',
        updatedAt: '2026-03-02T00:00:00.000Z',
        scores: {
          autonomy: 78,
          qualityTech: 75,
          costEfficiency: 71,
          designExcellence: 81,
          global: 76.55
        }
      },
      {
        id: 'AQCD-S050-W1',
        windowRef: 'S050-W1',
        updatedAt: '2026-03-03T00:00:00.000Z',
        scores: {
          autonomy: 80,
          qualityTech: 77,
          costEfficiency: 73,
          designExcellence: 83,
          global: 78.45
        }
      },
      {
        id: 'AQCD-S051-W1',
        windowRef: 'S051-W1',
        updatedAt: '2026-03-04T00:00:00.000Z',
        scores: {
          autonomy: 82,
          qualityTech: 79,
          costEfficiency: 74,
          designExcellence: 85,
          global: 80.15
        }
      }
    ],
    latencySamplesMs: [1100, 1400, 1700, 1800]
  };
}

describe('aqcd-readiness-rules unit', () => {
  it('builds readiness score rule-based v1 with visible contributing factors (S051/FR-047)', () => {
    const result = buildAqcdReadinessRules(buildPayload(), {
      nowMs: Date.parse('2026-03-04T01:00:00.000Z')
    });

    expect(result.allowed).toBe(true);
    expect(result.reasonCode).toBe('OK');
    expect(result.readiness).toMatchObject({
      model: 'AQCD_READINESS_RULES_V1',
      rulesVersion: 'S051-v1'
    });
    expect(result.readiness.factors).toHaveLength(4);
    expect(result.readiness.factors[0].rule).toBe('SNAPSHOT_COMPARATIVE');
    expect(result.diagnostics.rulesVersion).toBe('S051-v1');
  });

  it('returns threshold unmet with prioritized recommendations (S051/FR-048)', () => {
    const payload = buildPayload();
    payload.snapshots[2].updatedAt = '2026-03-20T00:00:00.000Z';

    const result = buildAqcdReadinessRules(payload, {
      nowMs: Date.parse('2026-03-26T00:00:00.000Z'),
      readinessV1Threshold: 90,
      enforceContinuity: false
    });

    expect(result.allowed).toBe(false);
    expect(result.reasonCode).toBe('AQCD_READINESS_THRESHOLD_UNMET');
    expect(result.recommendations.length).toBeGreaterThan(0);
    expect(result.recommendations[0]).toMatchObject({
      gate: 'G4'
    });
  });

  it('rejects invalid readiness rule weights', () => {
    const payload = buildPayload();
    payload.readinessV1Rules = {
      snapshotComparative: -1,
      trendStability: 25,
      continuityHealth: 25,
      freshnessHealth: 20
    };

    const result = buildAqcdReadinessRules(payload);

    expect(result.allowed).toBe(false);
    expect(result.reasonCode).toBe('AQCD_READINESS_RULES_INVALID');
    expect(result.correctiveActions).toContain('ALIGN_AQCD_READINESS_RULES');
  });

  it('is exported from index', () => {
    const result = buildAqcdReadinessRulesFromIndex(buildPayload(), {
      nowMs: Date.parse('2026-03-04T01:00:00.000Z')
    });

    expect(result.reasonCode).toBe('OK');
  });
});
