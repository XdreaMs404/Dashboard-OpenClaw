...
PROJECT_ROOT="$WORKSPACE_ROOT/bmad-total"
...
if [[ "$DRY_RUN" == "1" ]]; then
  changed="$(git status --porcelain -- bmad-total | wc -l | tr -d ' ')"
  ok "DRY_RUN: detected $changed pending file(s) under bmad-total for $SID"
  exit 0
fi
...
git reset -q -- bmad-total/runtime/git-sync 2>/dev/null || true
